...
export default function DashboardPage({ user }: { user: any }) {
    ...
    const fetchDataPrint = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/api/v1/student/${user.id}/printingLog`); // Replace with your API URL
            ...
            setTotalPrinted(result.studentInfo.totalPrinted);

            setDataPrint(result.returnData);
            setFilteredDataPrint(result.returnData); // Initialize filtered data
        } catch (error) {
    ...
    const handleDateChangePrint = (dates: any) => {
    ...
    const columnsPrint = [
        ...
        { title: 'Pages', dataIndex: 'numberOfPage', key: 'numberOfPage' },
